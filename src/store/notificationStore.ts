...
export interface Notification {
  ...
  title: string
  content: string
  type: 'info' | 'promotion' | 'urgent' | 'update' | 'package_created' | 'status_updated' | 'package_arrived' | 'new_message'
...
interface NotificationStore {
  ...
  updateNotification: (id: string, notification: Partial<Notification>) => Promise<boolean>
  deleteNotification: (id: string) => Promise<boolean>
  sendNotification: (notificationId: string, userIds: string[]) => Promise<boolean>
...
export const useNotificationStore = create<NotificationStore>((set, get) => ({
    ...
    try {
      ...
      if (error) {
        ...
        if (msg.includes("Could not find the table 'public.user_notifications'")) {
          ...
          const mapped = (legacy.data || []).map((n: any) => ({
            id: n.id,
  ...
  fetchPublicUsers: async (filters = {}) => {
    set({ loading: true, error: null })
    try {
      ...
      if (typeof (filters as any).minSpent === 'number') {
        query = query.gte('total_spent', (filters as any).minSpent)
      }
      if (typeof (filters as any).maxSpent === 'number') {
        query = query.lte('total_spent', (filters as any).maxSpent)
      }
      if (filters.dateFrom) {
        query = query.gte('created_at', filters.dateFrom)
      }
      ...
      if (!error) {
        set({ publicUsers: (data || []) as any, clientFilters: filters })
      } else {
        const msg = String(error.message || '')
        ...
        if (msg.includes("Could not find the table 'public.public_users'")) {
          ...
          const mapped: PublicUser[] = (r2.data || []).map((p: any) => ({
            id: p.id,
            name: [p.first_name, p.last_name].filter(Boolean).join(' ').trim() || 'Utilisateur',
  ...
  createNotification: async (notification) => {
    ...
    try {
      ...
      while (attempt < 5) {
        ...
        if (/violates check constraint/i.test(msg) && /notifications_type_check/i.test(msg)) {
          // pick next candidate type
          const next = tryOrder.shift()
          if (next && next !== payload.type) {
            payload.type = next
            attempt++
            continue
          }
    ...
    try {
      ...
      set(state => ({
        userNotifications: state.userNotifications.map(n => 
          n.id === userNotificationId ? { ...n, status: 'read' as const, read_at: new Date().toISOString() } : n
        )
      }))
